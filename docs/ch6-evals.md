...
這是面試的**鑑別題章**——「你怎麼知道你的系統真的有效？」答不好直接出局。讀完你能：(1) 講出四層 eval 架構；(2) 實際建一套 golden set 與 LLM-as-judge（W3 會為 Trilo 做一次）；(3) 把 eval 講成「AI 系統的測試套件」。
...
一句對頻工程師面試官的話：**「Eval set 就是 AI 系統的測試套件。沒有 eval 改 prompt，等於沒有測試改 production code。」**
...
- 「客戶說模型變笨了怎麼查？」→ 先跑 eval 定位：分數真降了嗎？哪類降？（接題庫 #10：drift 調查）
...
1. 面試官問：「你怎麼知道你的 AI 系統真的有效？」——四層架構完整作答（3 分鐘內）。
...
`interview/question-bank.md` 第七部分 #2（鑑別題本尊）、#10（變笨調查）；第四部分 #4（eval harness 設計）。W3 作業：為 Trilo 建 eval 跑出數字——完成後你答這章的所有題目都有「我做過」開頭。
