...
這是你的主武器章（Dcard JD 明列 MCP 實務經驗）。讀完你能：(1) 從 N×M 問題出發講清楚 MCP 為什麼存在；(2) 白板畫出完整架構；(3) 準備好 3 分鐘、30 分鐘、客戶版三種講法；(4) 把你自建 server 的經驗整理成面試證據。
...
實務上 tools 是主角（八成的 server 以 tools 為主），但面試要三個都講得出。
...
## 5.5 關鍵考題：MCP 與 Function Calling 的關係
...
延伸一句加分：「所以 MCP 是供應商中立的開放標準——這正是企業願意投資包 MCP server 的原因：這層投資不會因為換模型供應商而作廢。」
...
**W4 作業**：把你自建的 MCP servers 畫成一張架構圖（連了什麼系統、暴露哪些工具）存進 `interview/`，面試時這就是實物證據。
...
3. 「MCP 跟 function calling 什麼關係？」——用 5.5 的標準答案回答，不看稿。
...
`interview/question-bank.md` 第七部分 #5（主武器題）；Dcard JD 明列「理解 MCP 規格並具實際開發或整合經驗」——這章是你面 Dcard 的主場。
